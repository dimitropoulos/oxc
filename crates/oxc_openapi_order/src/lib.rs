...
//! single pre-order traversal: a table can order a node's own entries (the self arm), the
//! entries of each of its children (the child arm), or the entries of each element of an
//! array (the array arm). The three arms are mutually exclusive within one visit, but a
//! single mapping is reachable from two visits, its own and its parent's, so which order
//! survives depends on traversal order and on whether the traversal descends into the
//! deep copy the child arm makes.
//!
//! Transcribing that imports the bug. This crate inverts the question and asks it once
//! per mapping: which table orders this mapping's own entries? The answer is a pure
//! function of the mapping's ancestry, and a mapping is ordered exactly once, so
...
//! pass the resolved scalar value with quotes and escapes already removed, or a quoted
...
//!   `name`.
//!
//! One more comes from the caller rather than from here. [`resolve_root`] takes a content gate,
//! and both formatter backends pass "the root has an `openapi` key" where upstream tests
//! `if (jsonObj.openapi)`, plain JavaScript truthiness. So `openapi: ""`, `openapi: 0`,
//! `openapi: false`, `openapi: null` and (in YAML) a bare `openapi:` leave upstream's root
//! unordered and are root-ordered here: a document that names a version field without filling it
//! in is still an OpenAPI document, and a formatter should not decide otherwise. The differential
//! test models upstream's spelling, so this one divergence is pinned by the backends'
//! `openapi/falsy-openapi-value` fixtures rather than by the fuzz.
//!
