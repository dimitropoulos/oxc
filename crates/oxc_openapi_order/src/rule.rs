...
/// mapping's own step.
///
/// [`Step::Index`] steps are load-bearing, not decoration: upstream's guards index the
/// ancestry ABSOLUTELY (`path[0]`, `path[1]`, `path[3]`), and its own path includes array
/// indices as segments. Dropping them would shift every later index.
...
///
/// An override replaces one table by name and leaves every other table intact. This
/// DELIBERATELY differs from upstream's `--sortFile`, which replaces the whole set — there, a
/// sort file naming only `get` silently disables `requestBody`, `responses`, `properties` (so
/// child role stops working at all) and even `root`. Layering makes the option an escape hatch
/// for one table rather than an all-or-nothing switch.
///
/// A consequence of layering: a built-in table can never be removed, so
...
pub struct Options<'a> {
    ...
    /// strings, so listing `"0"` there does make the pass fire on the first element of a `components`
    /// sequence. That shape is not expressible in OpenAPI, and rule 2 already gives such an element
    /// the `components` table; see `a_components_sequence_element_takes_the_table_not_alphabetical`.
    ///
...
///
/// The same conflation applies to TABLE LOOKUP, where it is a deliberate narrowing rather than
/// an identity: upstream's path segments are strings even for sequence indices, so a table
/// literally named `"0"` would order every sequence's first element. No built-in table has a
/// numeric name, so this only bites a `keyOrder` override with one, which is meaningless for
/// OpenAPI.
...
/// guards exist to stop a schema property literally named `properties`, and inline example
/// payloads, from being reordered as if they were schemas.
///
...
pub fn resolve<'a>(options: &Options<'a>, path: &[Step<'_>]) -> Option<Table<'a>> {
    ...
    if matches!(path.last(), Some(Step::Index(_))) {
        ...
        // and rule 3 cannot apply. Neither alphabetical pass can either: both name a mapping
        // key, and this step is an index.
        return None;
    }

    // The opt-in alphabetical passes, BEFORE rule 3. Upstream runs them in the mapping's OWN
    // visit, which happens after its parent's; and its CHILD arm never writes its own node's
    // key order (it deep-copies the node, preserving the incoming order, and sorts only the
    // node's children). So an alphabetical write survives a rule-3 write.
    if let Some(table) = alphabetical(options, path) {
        return Some(table);
    }

    // Rule 3: the containing mapping is in child role and orders us.
...
/// The two opt-in alphabetical passes.
///
...
fn alphabetical<'a>(options: &Options<'a>, path: &[Step<'_>]) -> Option<Table<'a>> {
    ...
    // qualifies. Rule 2 already claims that shape — `components` always has a table, and
    // overrides can only replace one, never remove it — but the two functions should not have
    // to agree for this one to be right.
...
///
/// `has_truthy_openapi_member` is exactly that, not "is this an OpenAPI 3.x document": upstream
/// gates the root pass on JavaScript truthiness of the root's `openapi` member. So
/// `openapi: {}` IS root-ordered while `openapi: ""`, `openapi: 0`, `openapi: false` and
/// `openapi: null` are NOT — in YAML, that includes `openapi:`, `openapi: ~` and `openapi: no`.
/// `swagger: "2.0"` documents are never root-ordered; the root table is 3.x-shaped.
///
...
mod tests {
    ...
    #[test]
    fn rule_3_child_role_key_does_not_order_itself() {
    ...
    #[test]
    fn rule_3_child_role_key_is_still_ordered_by_its_own_parent() {
        // The brief's second named consequence: `responses > properties` means the
    ...
    #[test]
    fn rule_3_parent_must_be_the_direct_container() {
        // Across a sequence boundary there is no containing MAPPING, so rule 3 cannot reach
        ...
        // Same shape under a trio key whose child is a sequence: the reference's child arm
        // skips array-valued children, so nothing orders the element's members.
        assert_eq!(table_for("x.responses.a.#0"), None);
        assert_eq!(table_for("x.responses.a.#0.inner"), None);
    }

    #[test]
    fn a_trio_key_holding_a_sequence_orders_its_elements() {
        // The reference checks `Array.isArray(node)` BEFORE the child-role arm, so a trio key
        // whose value is a sequence takes the ARRAY arm and its ELEMENTS get the table.
    ...
    fn rule_1_beats_child_role_pre_order() {
        ...
        // its parent's (rule 3, the `properties` table). Upstream's traversal is pre-order,
    ...
    fn a_components_sequence_element_takes_the_table_not_alphabetical() {
        // A DELIBERATE narrowing, pinned with evidence that actually discriminates.
        //
