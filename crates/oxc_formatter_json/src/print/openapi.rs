...
mod tests {
    ...
    use crate::{JsonFormatOptions, format};
    ...
    /// ordering on against ordering off, so the two runs share Prettier's number normalization and
    /// the only difference left to measure is the reordering itself.
    #[test]
...
"#;
        ...
        for value in [
            ...
            assert!(ordered.contains(&format!(": {value}")), "{value} lost its bare spelling");
        }
    }
}
