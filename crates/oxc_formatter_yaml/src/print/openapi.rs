...
mod tests {
    ...
    use oxc_openapi_order::SortOpenapi;

    use crate::{YamlFormatOptions, format};

    /// Numbers reach the output as source text, never through an `f64`.
    ///
    /// openapi-format cannot manage this. It reads documents through a JavaScript object, so it
    /// rewrites large numbers to `<digits>===` sentinel strings before parsing and decodes them
    /// again on output. Its decode regex has no leading `-`, so negatives are encoded and never
    /// decoded, and the sentinel reaches the output as a quoted string. This has no encode step to
    /// get wrong, and this test is what keeps it that way.
    ///
    /// The `large-numbers` fixtures cover the same ground, but a snapshot can be regenerated by
    /// anyone with `INSTA_UPDATE=always` without reading the diff. This cannot: it compares
    /// ordering on against ordering off, so the two runs share whatever number normalization the
    /// printer applies and the only difference left to measure is the reordering itself.
    #[test]
    fn numbers_survive_reordering_as_text() {
        // Written `default, minimum, maximum, type` against a `properties` table of
        // `[description, type, items, format, example, default, enum]`, so every mapping moves.
        let source = "\
openapi: 3.1.0
info:
  title: t
    ...
    S:
      properties:
        a:
          ...
          example: 0.1000000000000000055511151231257827
          type: integer
        b:
          default: -80.56092644815843
          minimum: -1e400
          maximum: 1E10
          example: -0
          type: number
...
";
        ...
        let run = |enabled: bool| {
            ...
            let options = YamlFormatOptions {
                sort_openapi: SortOpenapi { enabled, ..SortOpenapi::default() },
        ...
        // Guards against passing vacuously: if reordering ever stops happening, the value
        // comparison below would hold trivially and prove nothing.
        assert_ne!(ordered, unordered, "the fixture stopped being reordered, so it tests nothing");

        let values = |code: &str| {
            let mut values: Vec<String> = code
                .lines()
        ...
        assert_eq!(
            values(&ordered),
            values(&unordered),
            "reordering changed a value; numbers must be carried as source text"
        );

        // The sentinel shape is a string on the way in and stays one, and no number acquires
        // quotes. Both are what upstream's decoder gets wrong, in opposite directions.
        assert_eq!(ordered.matches('"').count(), source.matches('"').count());
        for value in ["-9007199254740991", "-9007199254740993", "18446744073709551616", "-0"] {
            assert!(ordered.contains(&format!(": {value}\n")), "{value} lost its bare spelling");
        }
    }
}
